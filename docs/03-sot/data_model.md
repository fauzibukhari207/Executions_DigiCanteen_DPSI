...
Source: Derived from SRS v1.0 (SoT-1); menjadi acuan rancangan data-layer aplikasi (`lib/data/menu.ts`, `orders.ts`, `groups.ts`, `checkins.ts`, `points.ts`, `ratings.ts`, `lib/auth/session.tsx`)

---

## 1. Overview

Dokumen ini mendefinisikan model data DigiCanteen yang akan diimplementasikan di Supabase (PostgreSQL). Setiap tabel di bawah dirancang untuk dipetakan langsung ke fungsi query di `lib/data/*.ts` (kolom `snake_case` di database, dipetakan ke tipe `camelCase` di `lib/types.ts` lewat fungsi `mapRow`).
...
| items | JSONB | NOT NULL | Array `{menuItemId, quantity, note?, selectedOptions[]}` — kolom `note` harus didukung baik di tipe data maupun di UI keranjang, sehingga siswa benar-benar dapat mengisi catatan/permintaan khusus per item |
| subtotal | NUMERIC(12,2) | NOT NULL | Subtotal sebelum biaya layanan |
| service_fee | NUMERIC(12,2) | NOT NULL | Biaya layanan flat (BR-001) |
| discount | NUMERIC(12,2) | NOT NULL, DEFAULT 0 | Kolom untuk nilai diskon voucher yang diterapkan; harus diisi oleh `createOrder()` sesuai voucher yang dipilih siswa saat checkout — lihat Catatan Bagian 5.6 |
...
- ID menu harus divalidasi format UUID sebelum query untuk mencegah error keras dari ID yang tidak valid.
...
- **Catatan perancangan penting:** voucher yang sudah ditukar (`vouchers.is_used = false`) harus dapat dipilih dan dipakai untuk memotong total di halaman Checkout. `createOrder()` di `lib/data/orders.ts` wajib menerima parameter voucher dan mengisi `discount` sesuai nilainya, dan halaman `app/(siswa)/checkout/page.tsx` wajib menyediakan UI pemilihan voucher. Fitur "Poin & Voucher" (F007) baru dianggap lengkap jika voucher benar-benar terhubung ke alur pembayaran, bukan berhenti di tahap "voucher tersimpan & terlihat di `/poin`" saja — ini menjadi titik perhatian utama saat implementasi agar tidak terlewat.
