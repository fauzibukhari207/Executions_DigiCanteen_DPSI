...
|BR-006|**(Perhatian implementasi)** `app/(siswa)/checkout/page.tsx` harus mengirim `groupOrderId` berupa `group.id` (UUID) milik grup tersebut, **bukan** `groupCode` (kode 6 karakter, mis. "AB3CD9"). Karena `orders.group_order_id` didesain sebagai kolom UUID yang mereferensikan `group_orders.id`, mengirim kode string ke kolom ini akan gagal insert (error tipe data) atau menghasilkan relasi yang salah (tidak benar-benar FK ke baris `group_orders` yang valid). Ini perlu ditegaskan sejak awal implementasi agar fitur Group Order tidak bermasalah saat dianggap production-ready|
