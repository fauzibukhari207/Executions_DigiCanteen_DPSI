...
Dokumen ini mendefinisikan spesifikasi kebutuhan fungsional dan non-fungsional untuk **DigiCanteen**, aplikasi web pemesanan makanan/minuman kantin sekolah. Dokumen ini disusun sebagai acuan awal (*Source of Truth*, SoT-1) sebelum tahap implementasi dimulai, sehingga menjadi dasar bagi tim pengembang dalam merancang artefak turunan (User Flows, Data Model, Design System, Test Cases) dan membangun sistem.
...
* Keranjang belanja dengan kustomisasi opsi berbayar, termasuk catatan/permintaan khusus per item yang dapat diisi langsung dari antarmuka keranjang.
* **Group Order**: buat sesi pesan bareng (kode 6 karakter), gabung via kode/link, checkout kontribusi masing-masing anggota.
* Checkout: pilih slot waktu pengambilan, metode bayar (QRIS atau Saldo), penerapan voucher diskon jika tersedia.
* Pembayaran QRIS terintegrasi Xendit (dengan mode sandbox untuk kebutuhan pengujian bila API key produksi belum diisi) dan pembayaran Saldo (potong `wallet_balance`).
...
* Program Poin & Voucher: riwayat poin, tukar 100 poin menjadi voucher Rp5.000, dan penerapan voucher tersebut saat checkout.
...
* Manajemen multi-kantin/multi-sekolah dalam satu instance (seluruh penjual berbagi satu ruang data; belum ada pemisahan tenant).
...
* Aplikasi mobile native (rencana awal hanya web responsif).
...
* **Payment Gateway:** Xendit (QRIS), dengan mode sandbox otomatis (QR asli, auto "PAID" ±8 detik) sebagai fallback bila `XENDIT_SECRET_KEY` belum diisi.
...
* Setiap penjual akan mendaftar akun dan mengisi menunya sendiri sebelum siswa dapat memesan; tidak direncanakan adanya data seed otomatis.
* Bucket Storage `menu-images` perlu disiapkan di Supabase agar fitur upload foto menu dapat berfungsi.
* Realtime PostgreSQL perlu diaktifkan untuk tabel `orders` agar sinkronisasi otomatis berjalan; tanpa ini, aplikasi tetap dapat berjalan dengan mekanisme refresh manual sebagai fallback.

## 2.6 Constraints

* Karena seluruh penjual pada tahap awal berbagi satu ruang data, pemisahan data antar-toko (`seller_id`) perlu diuji menyeluruh setelah banyak akun penjual asli terdaftar.
* Akun Xendit yang akan digunakan kemungkinan belum terverifikasi bisnis pada tahap awal, sehingga QR yang dihasilkan pada mode API asli belum dapat menerima pembayaran sungguhan; perlu disediakan mekanisme simulasi pembayaran sebagai jalan tengah untuk pengujian end-to-end.
* Validasi kepemilikan menu perlu diterapkan baik di level UI (role-check dasar) maupun di level database (Row Level Security) agar konsisten untuk semua kasus tepi.
* Fitur voucher (F007) harus dirancang agar dapat diterapkan langsung pada proses checkout (memengaruhi kolom diskon pesanan), bukan sekadar riwayat penukaran poin — ini menjadi perhatian khusus saat perancangan alur checkout agar tidak berhenti di tahap "voucher tersimpan" saja.
...
| F003 | Keranjang & Checkout | Kelola item keranjang (disimpan lokal di sisi klien), pilih slot waktu, pilih metode bayar, buat pesanan. |
...
| F007 | Poin & Voucher | Riwayat poin, penukaran poin menjadi voucher diskon, dan penerapan voucher saat checkout. |
...
* Query menu dan pesanan harus memanfaatkan index alami Supabase (`created_at`, foreign key); halaman perlu menampilkan status "Memuat..." saat data belum tersedia, tanpa memblokir render awal.
* Kalkulasi total keranjang harus dioptimalkan agar query harga menu dilakukan sekali per kombinasi baris (bukan per baris satu-satu).

## 5.2 Security

* Autentikasi menggunakan Supabase Auth (email/password); peran disimpan di tabel `profiles`, tidak boleh dapat dipilih manual saat login.
* Row Level Security (RLS) harus membatasi penjual agar hanya dapat mengubah data miliknya; operasi lintas-peran sensitif (potong stok, ubah status pesanan dari sisi siswa) harus dilakukan lewat API route server yang memakai `SUPABASE_SERVICE_ROLE_KEY` (tidak pernah dikirim ke browser).
* Seluruh halaman siswa/penjual wajib dijaga komponen `RequireAuth`: belum login → redirect `/login`; role salah → redirect ke area yang sesuai.

## 5.3 Reliability & Data Integrity

* Endpoint konfirmasi pembayaran (`/api/orders/confirm-payment`) harus bersifat idempoten — pemanggilan berulang untuk pesanan yang sama tidak boleh memotong stok dua kali.
* Diperlukan validasi format UUID pada ID menu sebelum query ke database untuk mencegah error keras dari data yang tidak valid.
* Rating harus menggunakan mekanisme `upsert` dengan kunci unik (`order_id`, `menu_item_id`, `student_id`) sehingga pengiriman ulang menimpa rating lama, bukan menduplikasi.
...
* Navigasi harus konsisten lewat komponen `NavDrawer` yang tersedia di semua ukuran layar (bukan hanya `BottomNav` mobile).

## 5.5 Compatibility

* Checkout harus tetap dapat diuji end-to-end tanpa kredensial Xendit sungguhan lewat mode sandbox QR otomatis.
* Aplikasi tidak boleh crash meski `.env.local` kosong — halaman terproteksi menampilkan status memuat, bukan error.
