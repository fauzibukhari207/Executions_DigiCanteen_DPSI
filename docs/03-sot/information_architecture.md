...
Dokumen ini mendefinisikan Arsitektur Informasi (IA) DigiCanteen sebagai Source of Truth #2 (SoT-2), diturunkan dari SoT-1 (SRS v1.0) dan menjadi acuan rancangan struktur route App Router (`app/(auth)`, `app/(siswa)`, `app/(penjual)`, `app/profil`).
